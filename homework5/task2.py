...
# и смотрит, сколько раз каждый символ уже встречался и добавляет к символам с помощью постфикса формата _n.
...
def repeats(our_str):
    """ анализирует, сколько раз каждый символ уже встречался и добавляет это число к символам с помощью постфикса формата _n
            :param: our_str - исходная строка, которую принимает функция,
            :return: new_str - новая стока с постфиксом """
    new_str = ''
    for letter in our_str:  # t
        count = 0
        new_str = new_str + letter + '_'  # l_1e_1t_1t_2
        for letter_of_new_str in new_str:  # t
            if letter == letter_of_new_str:
                count = count + 1
        string_count = str(count)
        new_str = new_str + string_count
