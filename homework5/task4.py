...
import re
def scrabble(word):
    """ считает количество очков в словах, где каждая буква соответствует определенному кол-ву очков
                    :param: word - исходное слово,
                    :return: points - количество очков за слово """
    points = 0
    print(word)
    points_dict = {"а": 1, "в": 1, "е": 1, "ё": 1, "и": 1, "н": 1, "о": 1, "р": 1, "с": 1, "т": 1,
                   ...
                   "ю": 8, "щ": 10, "ъ": 15}
    for letter in word:
        buff = points_dict.get(letter)
        if buff is not None:
            points = points + points_dict.get(letter)
