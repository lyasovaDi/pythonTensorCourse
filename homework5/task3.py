...
def everything_for_your_cat(cats_data):
    """ оптимизирует хранение данных, сортируя котиков по их владельцам
                :param: cats_data - исходный список котиков и владельцев,
                :return: our_str - упорядоченный список котиков и влодельцев """
    cats_dict = {}
    for elements in cats_data:
        owner = (elements[2], elements[3])
        if owner not in cats_dict:
            cats_dict[owner] = [elements]
        else:
            cats_dict[owner].append(elements)

    our_str = ''
    for owner, cats in cats_dict.items():
        cat_info = []
        for elements in cats:
            cat_info.append(f"{elements[0]}, {elements[1]}")
        our_str += f"{owner[0]} {owner[1]}: {'; '.join(cat_info)}\n"
